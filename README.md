...
Each frame's content area (`regular-layout-frame::part(container)`) is `display: block` with a definite height, not a flex container, so `flex: 1` on slotted content silently does nothing. Give slotted content `height: 100%` to fill a panel.
